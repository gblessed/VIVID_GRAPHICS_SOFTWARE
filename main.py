...
def update_all_transaction_df():
    ...
    for i in range(len(new_all_transactions_df)):
        if str(item_database.getTaxableFromNameAndSP(new_all_transactions_df['Name'][i],
            ...
            t = Decimal(Decimal(new_all_transactions_df['S.Price'][i]) * Decimal(0.0000)).quantize(Decimal('.01'))
            taxes.append(t)
...
def get_tax_amount():
    ...
    return Decimal(Decimal(0.0000) * total_sale).quantize(Decimal('.01'))
...
def show_transaction_table(frame, frame1):
    ...
    for i in range(len(transaction_items)):
        data['row' + str(i + 1)] = {'Name': transaction_items[i][0], 'S.Price': transaction_items[i][1],
                                    'Date': transaction_items[i][2], 'Qty':transaction_items[i][4] }

        data1['row1'] = {'Total (#)': str(Decimal(transaction_items[i][4])*total_sale)}
...
def update_receipt_text():
    ...
    for i, n in enumerate(list_of_item_names):
        list_of_qtns.append(transaction_items[i][4])

    header = "         VIVID GRAPHICS LIMITED\n" +str(datetime.datetime.today()).split(' ')[0]+"\n\n"
    ...
    tax = Decimal(Decimal(0.0000) * subtotal).quantize(Decimal('.01'))
    ...
    for section in sections:
        ...
        section.page_width = Inches(2.84)
    header = doc.add_heading( "VIVID GRAPHICS LIMITED\n"+"               "+str(datetime.datetime.today()).split(' ')[0], 5) 
    ...
    subtotal = Decimal(0.0)
    # header2 = "\n"+f'{"Item_Name":<8}'  + f'{"Qty":<4}'+f'{"Amount":<7}'+"\n"
    # header2 = '{0: <10}{1:<4}{2:<7}\n'.format('Item_Name', 'Qty','Amount')

    # header2 = '{:15s} {:15s} {:15s} '.format("Item_Name","Qty","Amounts")
    ...
    for idx, item in enumerate(list_of_item_names):
        # header2 += f'{str(item):<8}' + f'{str(list_of_qtns[idx]):<4}' + "\t#" + f'{str(list_of_sp[idx]):<7}'+ "\n"
        # header2 += '\n{:15s} {:15f} {:15f} '.format(str(item),(list_of_qtns[idx]), (list_of_sp[idx]))
        row = table.add_row().cells
        row[0].text = str(item)
        row[1].text = str(list_of_qtns[idx])
        row[2].text = str(list_of_sp[idx])
    ...
    # for line in present_lines:
    #     header2 += line
    # para = doc.add_paragraph().add_run(header2)
    # para.font.size = Pt(8)
    doc.save('gfg.docx')
...
def add_item_to_transactions_databases(name, sp, date, op, payment_type='CREDIT', qty=3.0 ):
    """takes in necessary parameters and adds item into transactions table"""
    print('qty ->',qty)
    if name is not None:
        next(ON_OFF_CYC)
        if next(ON_OFF_CYC) == 'o':
            transactions_database.addData(name, op, date, payment_type, qty)
        else:
            transactions_database.addData(name, sp, date, payment_type, qty)
...
def get_misc_tot_w_tax(p, q):
    ...
    tax = Decimal(Decimal(0.0000) * Decimal(tot))
...
class TransactionPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.quantityEntry = tk.Entry(self.barcodeFrame, bg=ENTRY_COLOR, font=FONT, textvariable=quantityValue)
        self.quantityEntry.place(relx=0.01, rely=0.3, relheight=0.4, relwidth=0.1)
        self.quantityEntry.insert(0, 1.0)
        # Scan Barcode Label
        scan_label = tk.Label(self.barcodeFrame, text='Qty                    Item', bg=BACKGROUND_FRAME_COLOR, font=FONT,
                              fg='#fffafa')
        scan_label.place(relx=0.01, rely=0.1, relheight=0.14, relwidth=0.7)
        ...
        enter_btn = tk.Button(self.barcodeFrame, font=FONT, bg='#e8eb34', text='Enter',
                              command=lambda: [add_item_to_transactions_databases(
                                  ...
                                  str(datetime.datetime.today()).split(' ')[0],
                                  item_database.getOPFromBarcode(int(self.barcodeEntry.get())), qty = quantityValue.get()),
...
if __name__ == '__main__':
    app = InventoryApp()
    app.title("Vivid Graphics Receipt Printer")
    app.mainloop()
